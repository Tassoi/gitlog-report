...
- `src-tauri/src/services/git_service.rs` - Implemented diff generation, removed tz_offset_minutes
- `src-tauri/src/commands/git.rs` - Removed tz_offset_minutes parameter from commands

### Frontend (TypeScript)

- `src/components/Sidebar/RepoHistory.tsx` - Added data reloading on history item click

### Documentation

- `M2_REVIEW_FIXES.md` - This document

**Note**: All code changes were completed in commit `05cd360` (November 27, 2025). This commit adds documentation only.
